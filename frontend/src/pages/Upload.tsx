...
const MediaUpload: React.FC<UploadProps> = ({ module }) => {
  // Extract the specific loading states from the store
  const { uploadMedia, isUploadingVideo, isUploadingImage } = useMediaStore();
  
  // Combine them into a single boolean for the UI
  ...
  return (
    <div className="flex flex-col items-center p-4 border-2 border-dashed rounded-lg">
      <input
        type="file"
        // Restricting to images and videos as per your store logic
        accept="image/*,video/*"
        onChange={handleFileChange}
        className="hidden"
        ref={fileInputRef}
      ...
      <button
        onClick={() => fileInputRef.current?.click()}
        disabled={isUploading}
        className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-md disabled:opacity-50 transition-opacity"
      >
        {isUploading ? (
