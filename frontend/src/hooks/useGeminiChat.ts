...
export const useGeminiChat = ({ chatId, api, body }: UseGeminiChatProps) => {
  ...
  // Use underscore prefix for unused mock variables to satisfy TS rules
  const [apiStatus, _setApiStatus] = useState<"checking" | "ready" | "error">("ready");
  const [debugInfo] = useState({ hasOpenAIKey: !!api, keyLength: body ? 50 : 0 }); 
  ...
  const reload = useCallback(() => {
    if (messages.length > 0) {
      // Use spread and reverse for compatibility with older TS targets (ES6/ES2020)
      const lastUserMessage = [...messages].reverse().find((m: ChatMessage) => m.role === 'user');
      
      if (lastUserMessage) {
        const contentToResend = lastUserMessage.content;
        // Find index manually to support environments without findLastIndex
        const lastIndex = messages.map(m => m.id).lastIndexOf(lastUserMessage.id);
